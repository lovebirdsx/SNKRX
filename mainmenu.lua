...
function MainMenu:on_enter(from)
  ...
  self.arena_run_button = Button{group = self.main_ui, x = 55, y = gh/2 - 10, force_update = true, button_text = '开始挑战', fg_color = 'bg10', bg_color = 'bg', action = function(b)
    ...
    end, text = Text({{text = '[wavy, ' .. tostring(state.dark_transitions and 'fg' or 'bg') .. ']启动中...', font = pixul_font, alignment = 'center'}}, global_text_tags)}
  end}
  self.options_button = Button{group = self.main_ui, x = 47, y = gh/2 + 12, force_update = true, button_text = '选项', fg_color = 'bg10', bg_color = 'bg', action = function(b)
  ...
  self.discord_button = Button{group = self.main_ui, x = gw - 92, y = gh - 17, force_update = true, button_text = '加入Discord社区！', fg_color = 'bg10', bg_color = 'bg', action = function(b)
    ui_switch2:play{pitch = random:float(0.95, 1.05), volume = 0.5}
    b.spring:pull(0.2, 200, 10)
    b.selected = true
    ui_switch1:play{pitch = random:float(0.95, 1.05), volume = 0.5}
